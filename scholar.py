...
class ScholarArticle(object):
    ...
    def as_txt(self):
        ...
        for a in res:
            val2 = '#'
            if 'Citations list' in a:
                val2 = a.split(' ')[-1]
                break
        for a in res:
            if 'Title' in a:
                val3 = a.split('Title ')[-1]
                break
        print(val3)
...
def main():
    ...
    with open('_bibliography/papers.bib', 'r') as bibtex_file:
        lines = bibtex_file.readlines()
        paper_ids = []
        for line in lines:
            if line[0] == '@':
                paper_ids.append(line.split('{')[1].split(',')[0])

    print(paper_ids)
    # Send query to g scholar
    ...
    for i, entry in enumerate(bib_database.entries):
        phrase = entry['title'].split(' ')[0].lower().replace('{', '').replace(":", '')
        print(phrase)
        for res in results:
            title = res[0]
            if re.sub(r'\W+', '', title.split(' ')[0].lower()) == phrase:
                bib_database.entries[i]['citations'] = str(res[1])
                bib_database.entries[i]['citationlist'] = str(res[2])

    output = '---\n---\n\n'
    i = 0
    for paper_id in paper_ids:
        for i, entry in enumerate(bib_database.entries):
            if entry['ID'] == paper_id: break
        bib_database2 = bibtexparser.loads('')
        bib_database2.entries.append(bib_database.entries[i])
        output += bibtexparser.dumps(bib_database2)

    with open('_bibliography/papers.bib', 'w') as bibtex_file:
